  ...
  if (model$type == 'CHI') {
    ...
    for (i in model$Alpha[,]) {
      ...
      PLOT <- Theta %>%
        ...
        coord_flip()+
        theme_light()+
        ggtitle("Team rankings of this season - CHI model")+
        theme(
          panel.grid.major.y = element_blank(),
          panel.border = element_blank(),
          axis.ticks.y = element_blank())+
...
CHI_2024 = BT_Model(matdatanew,'CHI',R=R)
modelplot<-BT_plots(CHI_2024)
# 4
...
ProbabilitiesCHI <- BT_predict(CHI_2024, formaldatanew, R)

Seedsrun <- function() {
  Seeds <- PlayInTournament(CHI_2024, playin2024, R) 
...
Seeds_East = function(Seeds){
  ...
  Team <- c("Boston Celtics", "New York Knicks", "Milwaukee Bucks", 
            "Cleveland Cavaliers", "Orlando Magic", "Indiana Pacers",
            Seeds[1,2], Seeds[3,2], Seeds[5,2], Seeds[7,2])
...
East <- function() {
  result <- Simulation_Playoff_East(playoffs_East, Seedsofeast, ProbabilitiesCHI)
  EastResult <<- result
  return(EastResult)
}
...
West <- function() {
  result <- Simulation_Playoff_West(playoffs_West, Seedsofwest, ProbabilitiesCHI)
  WestResult <<- result
  return(WestResult)
}
#7 
...
Finalgame2 = function(){
  return(Simulation_Final(Finalteam, ProbabilitiesCHI))
  
}

NBAPLAYOFFS <- function() {
  # Combine all the functions to obtain results of the simulations
  Seeds <- PlayInTournament(CHI_2024, playin2024, R = R)  
  Seedsofeast <- Seeds_East(Seeds)
  EastResult <- East()  
  Seedsofwest <- Seeds_West(Seeds)
  WestResult <- West()  
